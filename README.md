...
Backend (Sunucu) Kurulumu Birinci Terminali açın: cd backend npm install (Eğer burada hata alırsanız yukarıda bulunan 🛠️ Ön Koşullar (Kurulumdan Önce Kontrol Edin) maddesine bakın) Sunucuyu başlatın: cd backend node server.js Sunucu şu adreste çalışacaktır: https://randevium-backend.onrender.com
